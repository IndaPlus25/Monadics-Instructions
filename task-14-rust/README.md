...
3) _If you're developing in Visual Studio Code_, install the _superior_ [Rust extension](https://marketplace.visualstudio.com/items?itemName=rust-lang.rust-analyzer).

### Prepare for your assigment

Your first assignment is turned in by uploading it to a repository named `<KTH_ID>-<task name>` (ex. `bwidman-rust`) under the `IndaPlus25` organisation. Be careful to get the spelling right.
...
1) Create a repository named `<KTH_ID>-<task name>`. (ex. `bwidman-rust`)
