...
fn run_compiler() -> Result<(), ErrorKind> {
    ...
    if args.len() < 2 {
        print_error(
            TAG_ARGUMENT_NOT_VALID,
            "A path to a BBVV file must be provided!",
        );
                ...
                while curr_i < lines.len() {
                    if line_has_code(&lines[curr_i]) {
                        match definitions.get(lines[curr_i].split_whitespace().next().unwrap()) {
                            ...
                            None => match compiler::run(lines[curr_i].as_str()) {
...
fn main() {
    ...
    std::process::exit(match run_compiler() {
        ...
        Err(_err) => {
            eprintln!(
                "Program exited on error {:?}.",
                if _err == ErrorKind::Other {
